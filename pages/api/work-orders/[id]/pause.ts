...
export default requireRole(["ADMIN", "MANAGER", "OPERATOR"], async (req, res) => {
  ...
  try {
    // Rate limit per IP per WO pause
    const ip = getClientIp(req as any);
    const rl = checkRateLimit(`wo:pause:${id}:${ip}`, 20, 60 * 1000);
    rateLimitResponse(res, rl.remaining, rl.resetAt);
    if (!rl.allowed) return res.status(429).json({ error: "Too many pause attempts, slow down" });
