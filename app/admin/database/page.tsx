...
export default function DatabaseViewerPage() {
  ...
  const fetchTableData = async (tableName: string, page: number = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/database/query?table=${tableName}&limit=${recordsPerPage}&offset=${offset}&search=${encodeURIComponent(searchTerm)}`)
  ...
  const totalPages = tableData ? Math.ceil(tableData.recordCount / recordsPerPage) : 0
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Direct access to database tables and records</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={exportTableData} disabled={!selectedTable}>
        ...
        <TabsContent value="tables" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tablesInfo.map((table) => (
              <Card 
                key={table.tableName} 
                className={`cursor-pointer transition-colors ${selectedTable === table.tableName ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-gray-50'}`}
              ...
              >
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{table.recordCount}</div>
                    ...
                    {totalPages > 1 && (
                      <div className="flex justify-between items-center">
                        <div className="text-sm text-gray-500">
                          Showing {((currentPage - 1) * recordsPerPage) + 1} to {Math.min(currentPage * recordsPerPage, tableData.recordCount)} of {tableData.recordCount} results
                        </div>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
